...
Well, we used Declarative Authorization for a while, but got stuck when it comes to name-spaces and inheritance. So, we investigated in the possibilities of using CanCan and CanTango, and found that CanCan could be slow, because all authorizations has to be loaded on every request. CanTango has tackled this problem by implementing cashing, but the framework is still evolving and seems fairly complex. At the same time, CanTango is role focused and not resource focused.

*What will you benefit from when using Trust?*

* Resource focused authorizations, not role focused
...
* Fast authorization loading, where no cashing is needed. All authorizations are declared on class level, so once loaded, they stay in memory.
...
* Loading datasets for the index action. You may use other plugins / gems for doing this, or you may implement youor own mechanism.
...
Create the authorization file in your model directory. Example
...
The can? method is accessible from the controller, the views and the models. Here are some coding examples:
...
On ActiveRecord objects
    ...
    @customer.can? :edit          # does the current user have permission to edit the given customer?
    Customer.can? :create         # does the current user have permission to create customers?
    @account.can? :edit, @client  # is current user allowed to edit the account associated with the client?
...
You may want to have a look at Trust::Controller to see what it is using to make your own customizations.
